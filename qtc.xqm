...
(:~
 ...
 : @param $odd the compiled(!) ODD belonging to the TEI files or the respective spec files from the TEI source
 : @param $class the class(es) to search for as xs:string (e.g. 'model.nameLike.agent')
...
declare function qtc:query-elements-by-class($source as item()*, $odd as document-node()*, $class as xs:string*) as element()* {
    for $elem in qtc:get-elements-from-class($odd, $class)
...
(:~
 ...
 :
 : @param $odd the compiled(!) ODD belonging to the TEI files or the respective spec files from the TEI source
 : @param $class the class(es) to search for as xs:string (e.g. 'att.global')
